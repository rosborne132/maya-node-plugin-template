...
MStatus CustomNode::initialize() {
    ...
    inputCurve = typedFn.create("inputCurve", "incv", MFnData::kNurbsCurve, MObject::kNullObj, &status);
    ...
    outputVal = numFn.create("outputValue", "ov", MFnNumericData::kDouble, 0, &status);
...
MStatus CustomNode::compute(const MPlug& plug, MDataBlock& dataBlock) {
    if (plug != outputVal) return MS::kSuccess;
